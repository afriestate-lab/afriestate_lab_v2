# Privacy Policy for Icumbi Mobile App

**Last Updated:** August 18, 2025  
**Effective Date:** August 18, 2025  
**App Version:** 1.0.0

## 1. Introduction

Welcome to Icumbi ("we," "our," or "us"). This Privacy Policy explains how we collect, use, store, and protect your personal information when you use the Icumbi mobile application ("App").

By using our App, you agree to the collection and use of information in accordance with this policy. If you do not agree with this policy, please do not use our App.

## 2. Information We Collect

### 2.1 Personal Information
We collect the following personal information when you use our App:

- **Full Name**: Your complete name as provided during registration
- **Phone Number**: Your 10-digit Rwandan phone number (primary identifier)
- **Email Address**: Your email address for account verification and communications
- **Password**: Securely encrypted password for account authentication
- **Role**: Your role in the platform (tenant, landlord, manager, admin)
- **Profile Picture**: Optional avatar image (if provided)

### 2.2 Property and Rental Information
When using our property management features, we collect:

- **Property Details**: Property names, addresses, descriptions, and specifications
- **Property Images**: Photos of properties and rooms (with your consent)
- **Rental Information**: Lease terms, payment amounts, and rental history
- **Location Data**: Property addresses and general location information

### 2.3 Payment Information
For payment processing, we collect:

- **Payment Method**: Your chosen payment method (MTN MoMo, Airtel Money, Bank Transfer, Cards, Cash)
- **Payment Amounts**: Transaction amounts and payment history
- **Transaction IDs**: Unique identifiers for payment tracking
- **Phone Numbers**: For mobile money payments (Airtel phone numbers)

### 2.4 Device and Technical Information
We automatically collect:

- **Device Information**: Device type, operating system, and app version
- **Usage Data**: App usage patterns, features accessed, and interaction data
- **Network Information**: Internet connection type and network status
- **Crash Reports**: App performance and error data (anonymized)

### 2.5 Location Information
With your explicit consent, we may collect:

- **GPS Location**: Precise location for property search and mapping
- **Location Permissions**: Access to location services for nearby property features

## 3. How We Use Your Information

### 3.1 Account Management
- Create and manage your user account
- Authenticate your identity and maintain account security
- Provide personalized user experience based on your role
- Send account-related notifications and updates

### 3.2 Property Management
- Display and manage property listings
- Process property bookings and reservations
- Handle tenant-landlord communications
- Generate property reports and analytics

### 3.3 Payment Processing
- Process rental payments and transactions
- Generate payment receipts and records
- Track payment history and financial reports
- Handle payment disputes and refunds

### 3.4 App Functionality
- Provide core app features and services
- Improve app performance and user experience
- Debug technical issues and maintain app stability
- Ensure app security and prevent fraud

### 3.5 Communication
- Send important notifications about your account
- Provide customer support and assistance
- Share updates about app features and services
- Send payment reminders and lease notifications

## 4. Information Sharing and Disclosure

### 4.1 With Your Consent
We may share your information with third parties only when you explicitly consent to such sharing.

### 4.2 Service Providers
We may share information with trusted service providers who assist us in:

- **Payment Processing**: MTN MoMo, Airtel Money, and banking partners
- **Cloud Storage**: Supabase for secure data storage
- **Analytics**: App usage analytics and performance monitoring
- **Customer Support**: Support ticket management and user assistance

### 4.3 Legal Requirements
We may disclose your information when required by law, including:

- **Legal Proceedings**: Court orders, subpoenas, or legal requests
- **Government Authorities**: Law enforcement or regulatory agencies
- **Safety and Security**: To protect user safety or prevent fraud
- **Compliance**: To comply with applicable laws and regulations

### 4.4 Business Transfers
In the event of a merger, acquisition, or sale of assets, your information may be transferred to the new entity, subject to the same privacy protections.

## 5. Data Security

### 5.1 Security Measures
We implement industry-standard security measures to protect your information:

- **Encryption**: All data is encrypted in transit and at rest
- **Authentication**: Secure login and multi-factor authentication
- **Access Controls**: Limited access to personal information
- **Regular Audits**: Security assessments and vulnerability testing

### 5.2 Data Retention
We retain your information for as long as necessary to:

- Provide our services and maintain your account
- Comply with legal obligations
- Resolve disputes and enforce agreements
- Improve our services and user experience

### 5.3 Data Deletion
You may request deletion of your account and personal information by contacting us at support@icumbi.com. We will process your request within 30 days.

## 6. Your Rights and Choices

### 6.1 Access and Control
You have the right to:

- **Access**: View and download your personal information
- **Correct**: Update or modify your information
- **Delete**: Request deletion of your account and data
- **Portability**: Export your data in a machine-readable format

### 6.2 Communication Preferences
You can control how we communicate with you:

- **Email Notifications**: Opt in/out of email communications
- **Push Notifications**: Manage app notification settings
- **SMS Messages**: Control SMS notifications for payments

### 6.3 Location Services
You can control location access:

- **Enable/Disable**: Turn location services on or off
- **Permission Management**: Manage location permissions in device settings
- **Granular Control**: Choose which features can access location

## 7. Children's Privacy

Our App is not intended for children under 13 years of age. We do not knowingly collect personal information from children under 13. If you are a parent or guardian and believe your child has provided us with personal information, please contact us immediately.

## 8. International Data Transfers

Your information may be transferred to and processed in countries other than Rwanda. We ensure appropriate safeguards are in place to protect your information during international transfers.

## 9. Third-Party Services

### 9.1 Payment Processors
Our payment processing partners include:

- **MTN Mobile Money**: Mobile money payments
- **Airtel Money**: Mobile money payments
- **Banking Partners**: Traditional bank transfers
- **Card Processors**: Credit/debit card payments

### 9.2 Analytics and Monitoring
We use third-party services for:

- **App Analytics**: Usage patterns and performance monitoring
- **Crash Reporting**: Error tracking and app stability
- **Customer Support**: Help desk and support management

## 10. Changes to This Policy

We may update this Privacy Policy from time to time. We will notify you of any material changes by:

- **In-App Notification**: Displaying a notice in the app
- **Email Notification**: Sending an email to your registered address
- **Website Update**: Posting the updated policy on our website

## 11. Contact Information

If you have questions about this Privacy Policy or our data practices, please contact us:

**Email:** support@icumbi.com  
**Phone:** +250 780 0566 266  
**Address:** Kigali, Rwanda  
**Website:** https://icumbi.com/privacy

## 12. Complaints and Disputes

If you have concerns about our data practices, you may:

1. **Contact Us**: Reach out to our privacy team
2. **Data Protection Authority**: File a complaint with the relevant authority
3. **Legal Action**: Pursue legal remedies as available under law

## 13. Governing Law

This Privacy Policy is governed by the laws of Rwanda. Any disputes arising from this policy will be resolved in accordance with Rwandan law.

## 14. Consent

By using the Icumbi mobile app, you consent to the collection, use, and sharing of your information as described in this Privacy Policy.

---

**Icumbi Mobile App**  
Version 1.0.0  
Last Updated: August 18, 2025 