    ...
    const handleSignIn = async (e)=>{
        ...
        try {
            ...
            if (!isEmail) {
                ...
                const { data: tenantUser } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('tenant_users').select('auth_user_id, email, role').eq('phone_number', cleanedPhone).single();
                if (tenantUser) {
                ...
                const { data: tenantUser } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('tenant_users').select('auth_user_id, email, role').eq('email', normalizedEmail).single();
                if (tenantUser) {
                    tenantUserRecord = tenantUser;
                }
            }
            ...
            if (data?.user) {
                ...
                const { data: userRecord } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('users').select('role').eq('id', data.user.id).single();
                if (userRecord?.role) {
                    resolvedRole = userRecord.role;
                }
                if (!resolvedRole) {
                    if (tenantUserRecord) {
                        resolvedRole = tenantUserRecord.role || 'tenant';
                    } else {
                        const { data: tenantRecord } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('tenant_users').select('role').eq('auth_user_id', data.user.id).single();
                        if (tenantRecord) {
                            resolvedRole = tenantRecord.role || 'tenant';
                        }
                    }
                }
                if (!resolvedRole) {
...
//# sourceMappingURL=_35cd4bf1._.js.map